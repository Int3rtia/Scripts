# OxyHub
- This is my first time seriously attempting to deobfuscate a Luarmor-protected script and yes, it was extremely difficult.
- I spent several hours tracing the loader, capturing its runtime stages, analyzing the payload, and untangling the nested execution flow just to recover parts of the script. Luarmor gave me a massive headache, and there were many points where I thought I had reached a dead end.
- The recovered code is still incomplete. It may not be as clean or as complete as the work produced by another hubs. Some functions, variable names, comments, and control flow had to be reconstructed manually, so this should **not** be considered the exact original source.

However, this is still meaningful progress for me. I managed to recover and understand parts of a Luarmor-protected payload on my first serious attempt, including some of its runtime behavior and the rhythm autoplay system.

> [!IMPORTANT]
> This is a **partial deobfuscation and manual reconstruction**, not a perfect recovery of the original source code. Any readable names or explanatory comments may have been added during analysis unless explicitly marked as recovered from the payload.
